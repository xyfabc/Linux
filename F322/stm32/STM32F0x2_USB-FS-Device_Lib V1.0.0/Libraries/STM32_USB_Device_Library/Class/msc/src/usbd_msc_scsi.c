...
/*add by eric.xi@hanvon at 20161107*/
static int8_t SCSI_HANVON(uint8_t lun , uint8_t *params);

static int8_t SCSI_HanvonProcessRead (uint8_t lun,uint8_t *params);
static int8_t SCSI_HanvonProcessWrite (uint8_t lun,uint8_t *params);

static int8_t SCSI_HanvonRead(uint8_t lun , uint8_t *params);
static int8_t SCSI_HanvonWrite (uint8_t lun , uint8_t *params);
                           ...
                           uint8_t *params)
{
  ...
  case SCSI_HANVON_CMD:
    return SCSI_HANVON(lun, params);
...
static int8_t SCSI_HanvonRead(uint8_t lun , uint8_t *params)
{
  if(MSC_BOT_State == BOT_IDLE)  /* Idle */
  {
	...
	SCSI_blk_len =  MSC_BOT_cbw.dDataLength; 
  ...
  return SCSI_HanvonProcessRead(lun,params);
}
...
static int8_t SCSI_HanvonWrite (uint8_t lun , uint8_t *params)
{
  ...
  {
    return SCSI_HanvonProcessWrite(lun,params);
  }
  return 0;
}

#define HANVON_READ                             0x00
#define HANVON_WRITE                            0x01



static int8_t SCSI_HANVON(uint8_t lun , uint8_t *params)
{
  if(HANVON_READ == params[1]){		//read
  	return SCSI_HanvonRead(lun, params);
  }else if(HANVON_WRITE == params[1]){	//write
	return SCSI_HanvonWrite(lun, params);
  }else{
      SCSI_SenseCode(MSC_BOT_cbw.bLUN, 
                     ILLEGAL_REQUEST, 
                     INVALID_CDB);
...
static int8_t SCSI_HanvonProcessRead (uint8_t lun,uint8_t *params)
{
	...
	u8 ret;
	u8 Commandword,addr1,addr2;
	Commandword = params[2];
	addr1 = params[3];
	addr2 = params[4];

  	memset(MSC_BOT_Data,0x00,MSC_MEDIA_PACKET);
  	len = MIN(SCSI_blk_len,MSC_MEDIA_PACKET); 
	if(len<=0){
		SCSI_SenseCode(lun, HARDWARE_ERROR, UNRECOVERED_READ_ERROR);
		return -1; 
	}
	ret = SyncCommandRead(Commandword,addr1,addr2,len,MSC_BOT_Data);
 	if(ret){
		SCSI_SenseCode(lun, HARDWARE_ERROR, UNRECOVERED_READ_ERROR);
		return -1; 
	}
...
static int8_t SCSI_HanvonProcessWrite (uint8_t lun,uint8_t *params)
{
  uint32_t len,i;
	...
	ret = SyncCommandWrite(Commandword,addr1,addr2,len,MSC_BOT_Data);
 	if(ret){
		SCSI_SenseCode(lun, HARDWARE_ERROR, WRITE_FAULT);
		return -1; 
	}
