	...
	unsigned char *pdata)
{
    ...
    while(1) {
       ...
       // for(i=0;i<1024*1024;i++);
        if(SyncCommandWrite(handle, endpoint_in, endpoint_out,Commandword,addr1,addr2,num,pdata)>=0){
            break;
        }
        if(i==1024){
            printf("CommandWriteWithRty erro\n");
            break;
        }
        i++;
	...
	unsigned char *pdata)
{
    ...
    while(1) {
        //for(i=0;i<5;i++);
        //usleep(10000);
        temp=SyncCommandRead(handle, endpoint_in, endpoint_out,Commandword,addr1,addr2,num,pdata);
        if(temp>0) {		
            break;
		}
        if(i==1024){
            printf("CommandReadWithRty erro\n");
            break;
        }
        i++;
...
static unsigned char memtest()
{
	...
	unsigned int i;
	tempa=0xaa;
    if(SyncCommandWrite(handle, endpoint_in, endpoint_out,CMD_SYSTEM_WRITE,0x0,ADDR_MTZ,1,&tempa))
		return 1;
	for(i=0;i<1024*1024;i++);
    if(SyncCommandRead(handle, endpoint_in, endpoint_out,CMD_SYSTEM_READ,0x0,ADDR_MTZ,1,&tempb))
		return 1;
...
int TestEncrypt( int *len1,int* len2 )
{
    ...
    ret = init_usb(VID,PID,&handle,&endpoint_in,&endpoint_out,&nb_ifaces);
    if(ret<0){
        //return -1;
    }
    #if 0
