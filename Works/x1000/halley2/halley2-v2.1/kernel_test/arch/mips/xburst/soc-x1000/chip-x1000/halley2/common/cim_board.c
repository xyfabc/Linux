...
static struct soc_camera_link iclink_0 = {
...
static struct soc_camera_link iclink_1 = {
    .bus_id     = 0,        /* Must match with the camera ID */
    .board_info = &jz_v4l2_camera_devs[BACK_CAMERA_INDEX],
    .i2c_adapter_id = 0,
    .power = front_camera_sensor_power,
    .reset = camera_sensor_reset,
};
...
struct platform_device mensa_front_camera_sensor[2] = {
	{
		.name	= "soc-camera-pdrv",
		.id	= -1,
		.dev	= {
			.platform_data = &iclink_0,
		},
	},
	{
		.name	= "soc-camera-pdrv",
...
static int __init mensa_board_cim_init(void) {
	...
	platform_device_register(&mensa_front_camera_sensor[0]);
	platform_device_register(&mensa_front_camera_sensor[1]);
#endif
