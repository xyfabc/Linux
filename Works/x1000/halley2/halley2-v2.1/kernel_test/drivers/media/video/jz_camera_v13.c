...
static unsigned int jz_camera_poll(struct file *file, poll_table *pt)
{
	...
	unsigned long temp = 0;
	unsigned int regval;
	int devnum = icd->devnum;

	if(!pcdev->mem[devnum].poll_flag)
		pcdev->mem[devnum].poll_flag = 1;

	buf = list_entry(icd->vb_vidq.stream.next, struct jz_buffer, vb.stream);

	if(pcdev->mem[devnum].is_first_start) {
		writel(0, pcdev->base + CIM_STATE);
		//HANVON_DBG("------------------is_first_start-----------------------icd->vdev.minor = %d----------------",icd->vdev->minor);

		/* please enable dma first, enable cim ctrl later.
		 * if enable cim ctrl first, RXFIFO can easily overflow.
		 */
		regval = (unsigned int)(pcdev->mem[devnum].dma_desc);
		...
		pcdev->mem[devnum].is_first_start = 0;
	}else{
/*
		int id=pcdev->mem[devnum].dma_desc_head->id;
		
		HANVON_DBG("------------------not_first_start-------------id = %d----------icd->vdev.minor = %d----------------",id,icd->vdev->minor);
		if(0 == id%4){
			regval = (unsigned int)(pcdev->mem[devnum].dma_desc);
			writel(regval, pcdev->base + CIM_DA);
		}
...
static int jz_camera_alloc_desc(struct jz_camera_dev *pcdev, struct soc_camera_device *icd, struct v4l2_requestbuffers *p)
{
	struct jz_camera_dma_desc *dma_desc_paddr;
	struct jz_camera_dma_desc *dma_desc;
	int devnum = icd->devnum;

	HANVON_DBG("++++++++++++++++++icd->vdev.mino++++++++++v4l2_requestbuffers count = %d::sizeof(*pcdev->dma_desc) * pcdev->buf_cnt = %d",p->count,sizeof(*pcdev->mem[devnum].dma_desc) * pcdev->mem[devnum].buf_cnt);

	pcdev->mem[devnum].buf_cnt = p->count;
	pcdev->mem[devnum].desc_vaddr = dma_alloc_coherent(pcdev->soc_host.v4l2_dev.dev,
			sizeof(*pcdev->mem[devnum].dma_desc) * pcdev->mem[devnum].buf_cnt,
			(dma_addr_t *)&pcdev->mem[devnum].dma_desc, GFP_KERNEL);

	dma_desc_paddr = (struct jz_camera_dma_desc *) pcdev->mem[devnum].dma_desc;
	dma_desc = (struct jz_camera_dma_desc *) pcdev->mem[devnum].desc_vaddr;

#if 1
	int i = 0;
	printk("pcdev->desc_vaddr = 0x%p, pcdev->dma_desc = 0x%p\n",pcdev->mem[devnum].desc_vaddr, (dma_addr_t *)pcdev->mem[devnum].dma_desc);
	for(i = 0; i < pcdev->mem[devnum].buf_cnt; i++){
...
static int jz_camera_reqbufs(struct soc_camera_device *icd, struct v4l2_requestbuffers *p) {
	...
	//HANVON_DBG("++++++++++++++++++icd->vdev.minor = %d++++++++++v4l2_requestbuffers count = %d",icd->vdev->minor,p->count);
	int i;
	struct soc_camera_host *ici = to_soc_camera_host(icd->dev.parent);
	struct jz_camera_dev *pcdev = ici->priv;
	...
	if(jz_camera_alloc_desc(pcdev,icd, p))
		return -ENOMEM;
...
static void free_buffer(struct videobuf_queue *vq, struct jz_buffer *buf) {
	...
	if(pcdev->mem[devnum].poll_flag)
		pcdev->mem[devnum].poll_flag = 0;

	if(pcdev->mem[devnum].vb_address_flag)
		pcdev->mem[devnum].vb_address_flag = 0;
}
...
static int jz_init_dma(struct videobuf_queue *vq, struct videobuf_buffer *vbuf) {
	...
	if(vbuf->i == 0) {
		pcdev->mem[devnum].dma_desc_head = (struct jz_camera_dma_desc *)(pcdev->mem[devnum].desc_vaddr);
	}

	if(vbuf->i == (pcdev->mem[devnum].buf_cnt - 1)) {
		...
		pcdev->mem[devnum].dma_desc_tail = (struct jz_camera_dma_desc *)(&dma_desc[vbuf->i]);
	} else {
		dma_desc[vbuf->i].next = (dma_addr_t) (&pcdev->mem[devnum].dma_desc[vbuf->i + 1]);
	}
		...
		struct videobuf_buffer *vb, enum v4l2_field field)
{
	...
	int devnum = icd->devnum;
//	HANVON_DBG("++++++++++++++++++icd->vdev.minor = %d++++devnum = %d++++++",icd->vdev->minor,devnum);
...
static void jz_videobuf_queue(struct videobuf_queue *vq, struct videobuf_buffer *vb)
{
	...
	//HANVON_DBG("++++++++++++++++++icd->vdev.minor = %d++++++++++",icd->vdev->minor);

	dma_desc_vaddr = (struct jz_camera_dma_desc *) pcdev->mem[devnum].desc_vaddr;

	vb->state = VIDEOBUF_ACTIVE;

	/* judged vb->i */
	if((vb->i > pcdev->mem[devnum].buf_cnt) || (vb->i < 0)) {
	...
	if(!pcdev->mem[devnum].vb_address_flag) {
		pcdev->mem[devnum].vb_address_flag = 1;
		for(i = 0; i < pcdev->mem[devnum].buf_cnt; i++)
			pcdev->mem[devnum].vb_address[i] = (struct  videobuf_buffer *)vq->bufs[i];
	}

	/* has select */
	if(pcdev->mem[devnum].poll_flag) {
		if(pcdev->mem[devnum].dma_desc_head != pcdev->mem[devnum].dma_desc_tail) {
			dma_desc_vaddr[vb->i].cmd |= CIM_CMD_STOP;
			dma_desc_vaddr[vb->i].cmd &= (~CIM_CMD_EOFINT);

			pcdev->mem[devnum].dma_desc_tail->next = (dma_addr_t) (&pcdev->mem[devnum].dma_desc[vb->i]);
			pcdev->mem[devnum].dma_desc_tail->cmd &= (~CIM_CMD_STOP);
			pcdev->mem[devnum].dma_desc_tail->cmd |=  CIM_CMD_EOFINT;
			pcdev->mem[devnum].dma_desc_tail = (struct jz_camera_dma_desc *)(&dma_desc_vaddr[vb->i]);
		} else {
			if(is_cim_disabled(pcdev)) {
				dma_desc_vaddr[vb->i].cmd |= CIM_CMD_STOP;
				dma_desc_vaddr[vb->i].cmd &= (~CIM_CMD_EOFINT);

				pcdev->mem[devnum].dma_desc_head = (struct jz_camera_dma_desc *)(&dma_desc_vaddr[vb->i]);
				pcdev->mem[devnum].dma_desc_tail = (struct jz_camera_dma_desc *)(&dma_desc_vaddr[vb->i]);

				dma_desc_paddr = (struct jz_camera_dma_desc *) pcdev->mem[devnum].dma_desc;

				/* Configure register CIMDCIM_DAA */
				
				regval = (unsigned int) (&dma_desc_paddr[vb->i]);
				//HANVON_DBG("regval = 0x%p",regval);
...
static int jz_camera_set_bus_param(struct soc_camera_device *icd, __u32 pixfmt) {
	...
	if(cfg_reg & CIM_CFG_PACK_Y0UY1V)
		cfg_reg |= CIM_CFG_BS0_2_OBYT0 | CIM_CFG_BS1_2_OBYT1 | CIM_CFG_BS2_2_OBYT2 | CIM_CFG_BS3_2_OBYT3;
...
static void jz_camera_deactivate(struct jz_camera_dev *pcdev,struct soc_camera_device *icd) {
	unsigned long temp = 0;
	if(!icd){
		return;
	}
	...
	if(pcdev && pcdev->mem[devnum].desc_vaddr) {
		dma_free_coherent(pcdev->soc_host.v4l2_dev.dev,
				sizeof(*pcdev->mem[devnum].dma_desc) * pcdev->mem[devnum].buf_cnt,
				pcdev->mem[devnum].desc_vaddr, (dma_addr_t )pcdev->mem[devnum].dma_desc);

		pcdev->mem[devnum].desc_vaddr = NULL;
	}
...
static int jz_camera_add_device(struct soc_camera_device *icd) {
	...
	dprintk(6, "jz Camera driver attached to camera %d\n",
			icd->devnum);
	HANVON_DBG("icd->devnum = %d",icd->devnum);

	pcdev->icd[icd_index] = icd;
	pcdev->mem[icd_index].is_first_start = 1;
	HANVON_DBG("++++++++++++icd_index = %d++++++++++++++++",icd_index);
...
static void jz_camera_remove_device(struct soc_camera_device *icd) {
	...
	jz_camera_deactivate(pcdev,icd);
...
//	cim_dump_reg(pcdev);
	...
	/* clear dma interrupt status */
		...
		writel(temp, pcdev->base + CIM_STATE);
		jz_camera_wakeup(pcdev, (pcdev->mem[devnum].vb_address[pcdev->mem[devnum].dma_desc_head->id]));
		...
		if(pcdev->mem[devnum].dma_desc_head == pcdev->mem[devnum].dma_desc_tail) {
			...
			HANVON_DBG("+++++++++++++CIM_STATE_DMA_STOP+++++++++2++id = %d++++",pcdev->mem[devnum].dma_desc_head->id);
			
			pcdev->mem[devnum].dma_desc_head =
				(struct jz_camera_dma_desc *)UNCAC_ADDR(phys_to_virt(pcdev->mem[devnum].dma_desc_head->next));

			/* Configure register CIMDA */
			dma_desc_paddr = (struct jz_camera_dma_desc *) pcdev->mem[devnum].dma_desc;
			regval = (unsigned int) (&dma_desc_paddr[pcdev->mem[devnum].dma_desc_head->id]);
			writel(regval, pcdev->base + CIM_DA);

			if(CIM_CAMERA_FRONT == devnum){	
				wake_up_interruptible(&wq_out_front);
			}else if(CIM_CAMERA_BACK == devnum){
				wake_up_interruptible(&wq_out_back);
			}
			sensor_select++;
		}
	...
	if(status & CIM_STATE_DMA_EOF) {
		/* clear dma interrupt status */
		//HANVON_DBG("+++++++++++++CIM_STATE_DMA_EOF+++++++++++++++");
		temp = readl(pcdev->base + CIM_STATE);
		temp &= (~CIM_STATE_DMA_EOF);
		writel(temp, pcdev->base + CIM_STATE);

		if(CIM_CAMERA_FRONT == devnum){	
			//HANVON_DBG("++++++EOF+++++++CIM_CAMERA_FRONT+++++++sensor_select = %d++++devnum = %d++++",sensor_select,devnum);
			wake_up_interruptible(&wq_out_front);
		}else if(CIM_CAMERA_BACK == devnum){
			//HANVON_DBG("++++++EOF+++++++CIM_CAMERA_BACK++++++++sensor_select = %d++++devnum = %d++++",sensor_select,devnum);
			wake_up_interruptible(&wq_out_back);
		}

		jz_camera_wakeup(pcdev, (struct videobuf_buffer *)(pcdev->mem[devnum].vb_address[pcdev->mem[devnum].dma_desc_head->id]));
		if(pcdev->mem[devnum].dma_desc_head != pcdev->mem[devnum].dma_desc_tail) {
			pcdev->mem[devnum].dma_desc_head =
				(struct jz_camera_dma_desc *)UNCAC_ADDR(phys_to_virt(pcdev->mem[devnum].dma_desc_head->next));
		}
		HANVON_DBG("+++++++++++++EOF+++++devnum=%d++++++id = %d++++",devnum,pcdev->mem[devnum].dma_desc_head->id);
...
*/
	...
	for(i=0;i<MAX_SOC_CAM_NUM;i++){
		pcdev->mem[i].poll_flag 	= 0;
		pcdev->mem[i].vb_address_flag		= 0;
	}
	...
	init_waitqueue_head(&wq_out_front);
	init_waitqueue_head(&wq_out_back);
/*
	if (!IS_ERR(pcdev->soc_host.regul))
		    regulator_disable(pcdev->soc_host.regul);
