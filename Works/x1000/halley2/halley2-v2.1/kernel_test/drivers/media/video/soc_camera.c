...
wait_queue_head_t          wq_out_front;
wait_queue_head_t          wq_out_back;

EXPORT_SYMBOL_GPL(wq_out_front);
EXPORT_SYMBOL_GPL(wq_out_back);
			   ...
			   struct v4l2_buffer *p)
{
	struct soc_camera_device *icd = file->private_data;
	struct soc_camera_host *ici = to_soc_camera_host(icd->dev.parent);
	//HANVON_DBG("++++++++++++++++++++++++++++++++++devnum = %d",icd->devnum);
			    ...
			    struct v4l2_buffer *p)
{
	...
	if (ici->ops->init_videobuf){
		...
		if(CIM_CAMERA_FRONT == devnum){
			time = interruptible_sleep_on_timeout(&wq_out_back,1000);
			if (time < 1 ){
				printk("interruptible_sleep_on_timeout IMINOR_YUV time is:%ul\n",time);			
				return -1;				
			}
		}else if(CIM_CAMERA_BACK == devnum){
			time = interruptible_sleep_on_timeout(&wq_out_front,1000);
			if (time < 1 ){
				printk("interruptible_sleep_on_timeout IMINOR_YUV time is:%ul\n",time);			
				return -1;				
			}
		}
		//HANVON_DBG("++++++++++++++++ok++++++++++++++++++devnum = %d",icd->devnum);
...
static int soc_camera_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	HANVON_DBG( "mmap called, vma=0x%08lx\n", (unsigned long)vma);
	...
	dev_dbg(&icd->dev, "vma start=0x%08lx, size=%ld, ret=%d\n",
		...
		err);
	HANVON_DBG( "vma start=0x%08lx, size=%ld, ret=%d\n",
		(unsigned long)vma->vm_start,
		(unsigned long)vma->vm_end - (unsigned long)vma->vm_start,
		err);
...
{
	HANVON_DBG("++++++++++++++++++++++++++++++++++iminor(file->f_dentry->d_inode) = %d", iminor(file->f_dentry->d_inode));

	if(0 ==  iminor(file->f_dentry->d_inode)){
		HANVON_DBG("I am the camera 0!!!!!");

	}else if(1 ==  iminor(file->f_dentry->d_inode)){
...
static int __devinit soc_camera_pdrv_probe(struct platform_device *pdev)
{
	...
	icd = kzalloc(sizeof(*icd), GFP_KERNEL);     //kmalloc and  memset
	if (!icd)
		return -ENOMEM;
...
static int __init soc_camera_init(void)
{
	...
	ret = platform_driver_probe(&soc_camera_pdrv, soc_camera_pdrv_probe);   //camera sensor
	if (ret)
		goto epdr;
