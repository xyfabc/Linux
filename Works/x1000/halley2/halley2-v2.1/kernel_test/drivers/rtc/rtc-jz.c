...
static int wakeup_status_read_proc(char *page, char **start, off_t off,
                                     int count, int *eof, void *data)
{
	int len = 0;
	len += sprintf(page + len,"RTC_HWRSR = 0x%08x\n", inl(RTC_IOBASE + RTC_HWRSR));
	return len;
}

static int wakeup_status_write_proc(struct file *file, const char __user *buffer,
                                      unsigned long count, void *data)
{
	int hwrsr;

	sscanf(buffer, "0x%x\n", &hwrsr);
	jzrtc_write_reg(RTC_HWRSR, hwrsr);
...
static int jz_rtc_probe(struct platform_device *pdev)
{
	...
	res = create_proc_entry("wakeup_status", 0444, p);
	if (res) {
		res->read_proc = wakeup_status_read_proc;
		res->write_proc = wakeup_status_write_proc;
	}
